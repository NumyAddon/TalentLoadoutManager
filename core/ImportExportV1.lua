...
local LEVELING_BIT_WIDTH_DATA = 7; -- allows for 128 order indexes
...
--- @return table<number, TLM_LevelingBuildEntry> # [level] = entry
--- @private
...
function ImportExport:TryExportBlizzardLoadoutToString(configID, specID)
    ...
    if importStream:ExtractValue(BIT_WIDTH_HEADER_VERSION) ~= 1 then
    ...
    exportStream:AddValue(BIT_WIDTH_HEADER_VERSION, 1);
