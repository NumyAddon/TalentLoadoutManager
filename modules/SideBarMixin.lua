...
function SideBarMixin:OnInitialize()
    local loadoutNameSubText = L["Anything before the first '||' character will not display. This allows you to sort loadouts by adding a prefix."];
...
function SideBarMixin:CreateImportDialog()
    ...
    importControl.labelText = HUD_CLASS_TALENTS_IMPORT_DIALOG_EDIT_BOX_LABEL .. " " .. L["(Icy-veins calculator links are also supported)"];
    ...
    if addAutoApplyCheckbox then
        ...
        checkbox:SetScript('OnEnter', function(cb)
            ...
            GameTooltip:AddLine(L['If checked, the loadout will automatically be applied to your character when you import it.'], 1, 1, 1, true);
            GameTooltip:Show();
        end);
...
function SideBarMixin:CreateSideBar()
    ...
    sideBar.ImportButton:SetText(HUD_CLASS_TALENTS_IMPORT_LOADOUT_ACCEPT_BUTTON);
    ...
    sideBar.ToggleSideBarButton:SetScript("OnEnter", function()
        ...
        GameTooltip:AddLine(L["|cffeda55fShift + Click|r to move the side bar to the other side of the UI."], 1, 1, 1, true);
        GameTooltip:AddLine(L["|cffeda55fRight-Click|r to move the side bar to the inside side of the UI."], 1, 1, 1, true);
        GameTooltip:Show();
    end);
    --- @param mouseButton MouseButton
...
function SideBarMixin:CreateScrollBox(parentContainer)
    ...
    local function OnListElementInitialized(frame, entry)
        ...
        frame:SetScript("OnEnter", function()
            ...
            GameTooltip:AddLine(string.format(L["Left-Click to %s this loadout"], defaultAction), 1, 1, 1);
...
function SideBarMixin:GenerateMenu(rootDescription, frame, loadoutInfo)
    ...
    lock:SetTitleAndTextTooltip(L["Lock loadout"], L["Locking a loadout blocks you from saving changes to it."]);
    ...
    if loadoutInfo.playerIsOwner then
        ...
        rootDescription:CreateButton(L["Permanently hide all loadouts from this character"], function()
            ...
            ns.TLM:Printf(L["Loadouts from %s are now hidden. You can reset this in the config."], loadoutInfo.owner);
        end);
    end
end
